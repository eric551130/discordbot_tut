...
class GuessAB(Cog_Extension):
    ...
    async def ABstart(self,ctx):
        ...
        self.num = random.sample(self.L, 4)
        #print(self.start)
        #print(self.time)
        #print(self.L)
        print(self.num)
        ...
        def check(msg: discord.Message):
            n = str(msg.content)
            return msg.author == ctx.author and msg.channel == ctx.channel and (len(n)==4 and n[0]!=n[1]!=n[2]!=n[3])
        while self.time > 0:
            #print(F'ur time {self.time}')
            await ctx.send(F'{ctx.author} You have {self.time} chances left')
            ...
            if len(n) != 4:
                ...
                for i in range(4):
                    for j in range(4):
                        # print(n[i],num[j])
                        if int(n[i]) == int(self.num[j]) and i == j:
                            A += 1
                        elif int(n[i]) == int(self.num[j]) and i != j:
                            B += 1
                if A == 4:
                    #print('success')
                    self.start = False
        ...
        if self.win:
            ...
            await ctx.send(F'{ctx.author} the number is {self.num}\n:-1: Game Over，get out :-1:')
            self.start = False
            self.time = 0
        print(self.start)
        print(self.time)
